...
totalDays = 0 #number of times the first day of january is sunday
...
for year in years:
	...
	for monthIndex,month in enumerate(monthDays):
		for i in range(month):
			#we have to start at 1900 because that's when we have the date of a monday
			#but we only want to count sundays if the year is after 1900
			if year > 1900:
				if weekDayIndex == 0: #if it's sunday
					if i == 0: #if it's sunday and the first day of the month
